...
class MeshDrawer {
	...
	draw(trans) {
		...
		// Similar to the above code, we need to handle the normal:
		gl.bindBuffer(gl.ARRAY_BUFFER, this.normalbuffer);
		...
		// Set the value of the variable "vec3 lightPos":
	...
	setTexture(img) {
		...
		if (isPowerOf2(img.width) && isPowerOf2(img.height)) {
			...
			// Clamps the horizontal coordinates that are outside of the (i.e. higher than the) 0-1 range to the edge.
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
			// Clamps the vertical coordinates that are outside of the (i.e. higher than the) 0-1 range to the edge.
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
			// Smooth (Linear) Minification filter for zooming out:
			gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
			// Smooth (Linear) Magnification filter for zooming in:
...
const meshFS = `
			...
			uniform float ambient; // Represents the ambient light density.
			...
			void main()
			{
				if(showTex && enableLighting){
					...
					// Modulate texture color with light:
